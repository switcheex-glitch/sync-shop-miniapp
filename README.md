...
Оплата — через шлюз **Platega** (СБП и криптовалюта) с подтверждением по вебхуку.
...
- В разделе «Купить» пользователь выбирает способ оплаты (СБП / крипта) и нажимает «Оплатить».
...
- Создание транзакции: `POST https://app.platega.io/transaction/process`, заголовки `X-MerchantId` / `X-Secret`.
  Методы: **2 = СБП**, **10 = карта РФ (МИР/Visa/MC)**, **13 = криптовалюта**. В `payload` кладём id нашей покупки.
